...
import { computeArmitResult } from '../utils/calcEngine.js'
...
import { formatUSD, formatUSDPerBbl, formatBpd } from '../utils/formatters.js'
...
export default function CrudeAdvisor() {
  ...
  const copySummary = async () => {
    ...
    const winnerLine =
      diff <= 0.5
        ? `Margins are comparable — within ${formatUSDPerBbl(0.5)}.`
        : `${marginA > marginB ? `Option A (${derivedA.crude.name})` : `Option B (${derivedB.crude.name})`} is the better choice by ${formatUSDPerBbl(diff)} (${formatUSD(diff * throughputBpd)}/day).`

    const summary = `MarginIQ Crude Switching Advisor — Comparison Summary
Throughput: ${formatBpd(throughputBpd)}

Option A: ${derivedA.crude.name}
  Delivered cost: ${formatUSDPerBbl(derivedA.deliveredCost)}
  Net margin: ${formatUSDPerBbl(marginA)}

Option B: ${derivedB.crude.name}
  Delivered cost: ${formatUSDPerBbl(derivedB.deliveredCost)}
  Net margin: ${formatUSDPerBbl(marginB)}
    ...
    <div className="px-4 py-10 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mt-10 space-y-6">
          ...
          <div className="flex justify-center rounded-2xl border border-white/10 bg-armit-card p-8">
            <PageCta to="/stress-tester">Run Stress Test</PageCta>
          </div>
        </div>
      </div>
    </div>

import { formatUSD, formatUSDPerBbl, formatPct } from '../../utils/formatters.js'
    ...
    <div className={`rounded-lg border p-4 ${accent === 'teal' ? 'border-armit-teal/30' : 'border-armit-amber/30'}`}>
      ...
      <dl className="mt-3 space-y-2 text-sm">
        <div className="flex justify-between">
          ...
          <dd className="text-armit-text">{formatUSDPerBbl(option.fob)}</dd>
        </div>
        <div className="flex justify-between">
          <dt className="text-armit-muted">Freight + handling</dt>
          <dd className="text-armit-text">{formatUSDPerBbl(freightHandling)}</dd>
        </div>
        <div className="flex justify-between border-t border-white/10 pt-2 font-semibold">
          <dt className="text-armit-text">Total delivered</dt>
          <dd className="text-armit-text">{formatUSDPerBbl(option.deliveredCost)}</dd>
        </div>
        <div className="flex justify-between">
          <dt className="text-armit-muted">Freight as % of gross margin</dt>
          <dd className="text-armit-text">{formatPct(freightPctOfMargin)}</dd>
        </div>
      </dl>
    </div>
  )
  ...
  return (
    <div className="rounded-xl border border-white/10 bg-armit-card p-5">
      ...
      <p className="mt-4 text-xs text-armit-muted">
        ...
        <span className="font-semibold text-armit-text">{formatUSD(throughputBpd)}</span>/day in
        margin impact at your current throughput.
      </p>
    </div>
  )
}

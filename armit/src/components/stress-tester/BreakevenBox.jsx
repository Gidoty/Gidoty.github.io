      ...
      <div className="mt-4 space-y-4 text-sm">
        <div className="rounded-lg border border-white/10 bg-armit-bg/60 p-4">
          <p className="text-armit-muted">
            ...
            <span className="font-semibold text-armit-text">{formatUsd2(crudeBreakeven)}/bbl</span>.
          </p>
          <p className="mt-1 text-armit-muted">
            Current crude is <span className="font-semibold text-armit-text">{formatUsd2(currentCrude)}/bbl</span>{' '}
            — you have <span className={`font-semibold ${tone.text}`}>{formatUsd2(Math.abs(cushion))}/bbl</span>{' '}
        ...
        <div className="rounded-lg border border-white/10 bg-armit-bg/60 p-4">
          <p className="text-armit-muted">
            ...
            <span className="font-semibold text-armit-text">{formatUsd2(agoBreakeven)}/bbl</span> to break even at
            the BASE crude cost of {formatUsd2(currentCrude)}/bbl.
          </p>
          <p className="mt-1 text-xs text-armit-muted">
            Currently priced at {formatUsd2(currentAgo)}/bbl.
          </p>
        </div>
      </div>
    </div>

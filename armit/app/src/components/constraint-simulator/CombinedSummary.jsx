    ...
    <div className="space-y-6">
              ...
              {interventions.map((item) => (
                <tr key={item.name} className="border-b border-white/5 last:border-0">
                  ...
                  <td className="py-2.5 pr-0 text-armit-muted">
                    {item.roi === null ? 'N/A' : formatPct(item.roi)}
                  </td>
                </tr>
              ))}
            </tbody>
            <tfoot>
              <tr className="border-t border-white/10 font-semibold">
                ...
                <td className="py-2.5 pr-0 text-armit-teal">
                  {totalCost > 0 ? formatPct(((totalAnnual - totalCost) / totalCost) * 100) : 'N/A'}
                </td>
              </tr>
            </tfoot>
          </table>
      ...
      <div className="rounded-2xl border-2 border-armit-teal/40 bg-armit-teal/5 p-8">
        ...
        <div className="mt-6 grid grid-cols-1 gap-6 text-center sm:grid-cols-2 lg:grid-cols-5">
          <div>
            <div className="whitespace-nowrap text-xl font-bold text-armit-teal">{formatUsd0(totalDaily)}</div>
            <div className="mt-1 text-xs text-armit-muted">Total daily margin improvement</div>
          </div>
          <div>
            <div className="whitespace-nowrap text-xl font-bold text-armit-teal">{formatUsd0(totalAnnual)}</div>
            <div className="mt-1 text-xs text-armit-muted">Total annual gain</div>
          </div>
          <div>
            <div className="whitespace-nowrap text-xl font-bold text-armit-text">{formatUsd0(totalCost)}</div>
            <div className="mt-1 text-xs text-armit-muted">Total investment required</div>
          </div>
          <div>
            <div className="whitespace-nowrap text-xl font-bold text-armit-text">
          ...
          <div>
            <div
              className={`whitespace-nowrap text-xl font-bold ${firstYearNet >= 0 ? 'text-armit-emerald' : 'text-armit-coral'}`}
            >
